<?php
    ...
    class DiscourseAPI
    {
         ...
         **/
        public function createTopic(string $topicTitle, string $bodyText, string $categoryId , string $userName, int $replyToId = 0, $tag)
        {
            $params = [
                'title'                => $topicTitle,
                'raw'                  => $bodyText,
        ...
        public function getTopicFromSearch($topicTitle)
        {       
            $query = $topicTitle  . " in:title";
            $data = $this->_getRequest("/search.json", ["q" => $query], 'system', 'GET', false);
            foreach ($data->apiresult->topics as $topic) 
            {
                if( strtolower($topicTitle) == strtolower($topic->title) )
                {
                    return $topic;
                }
            }
         ...
         *
         **/
        private function _getRequest(string $reqString, array $paramArray = [], string $apiUser = 'system', $HTTPMETHOD = 'GET', $showEmails = true): \stdClass
        {
            $showEmails && $paramArray['show_emails']  = 'true';
            $ch                         = curl_init();
